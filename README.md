# CMU Ballbot LQR vs MPC Controller Comparison
...
MPC accelerates to the limit, cruises there, and brakes. During the cruise the body holds a constant $-0.23°$ lean. That is the lean needed for gravity to balance ball–ground friction at constant speed, and it matches the steady-state prediction from the linear model:

$$
\alpha = \frac{\mu_\theta}{m_B g \ell}\,\dot\theta = 0.228°
$$
...
- **Lean** $\lvert\theta + \phi\rvert \le 0.3°$. Lean is the unstable, unactuated direction, so it can only be limited by planning ahead through the dynamics.
